...
| ecdsa_pubkey | bytea | 主公钥(公开;用于回传前验签) |
...
- `request_approvals (request_id)`;`request_events (request_id, at)`;`push_tokens (group_id, member_id)`。
...
### 7.2 独立加密专测(用户裁定 2026-05-18,与 E2E-001 解耦)
