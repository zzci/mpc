...
| **多 group(一个设备多个钱包)** | ✅ 2026-05-21 SDK 多 group + walletcli `groups` / `pairings.json` / `/groups` UI + admin `/devices` 跨组视图 | `internal/mobileapi/sdk.go`, `internal/walletcli/{pair,groups_test}.go`, `internal/server/admin/devices.go` |
